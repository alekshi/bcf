...
For any questions, comments or ideas please contact by aleksandr.shilkin@bigswitch.com
...
class ClassScriptConfig():
  ...
  def __init__(self):
    ...
    self.analytics_node_search_url = '/es/api/dhcp-*/_search'
...
class ClassAnalyticsNode(ClassScriptConfig):
  ...
  def return_rogue_dhcp_servers_list(self):
    ...
    response = self.api_request(session_cookie = self.session_cookie, 
                                path = self.analytics_node_search_url, 
                                api_type = 'Analytics',
  ...
  def return_affected_dhcp_clients_list(self, server_ip):
    ...
    if int(response['hits']['total']) > 0:
      for dhcp_client in response['hits']['hits']:
        if dhcp_client['_source']['yiaddr'] not in affected_dhcp_client_list:
...
def main():
    ...
    analytics_node = ClassAnalyticsNode()
    bcf_controller = ClassBCFController()

    # Return list of DHCP servers IP addresses (except legal) what produced DHCP OFFER in the period.
    rogue_dhcp_servers_list = analytics_node.return_rogue_dhcp_servers_list()
    ...
    for rogue_dhcp_server_IP in rogue_dhcp_servers_list:
      rogue_dhcp_server_l2_details = bcf_controller.return_endpoint_mac_and_segment_by_ip(rogue_dhcp_server_IP)
      ...
      for rogue_dhcp_server_mac_and_segment in rogue_dhcp_server_l2_details:
        dhcp_server = bcf_controller.return_endpoint(endpointMAC = rogue_dhcp_server_mac_and_segment['mac'],
        ...
        if dhcp_server.attachment_point_state != 'static':
          bcf_controller.number_of_blocked_dhcp_servers+=1
          if dhcp_server.make_me_static(session_cookie = bcf_controller.session_cookie,
                                        endpointName = '{}_{}'.format(bcf_controller.rogue_dhcp_server_prefix,
                                                                   bcf_controller.number_of_blocked_dhcp_servers),
          ...
          affected_dhcp_clients = analytics_node.return_affected_dhcp_clients_list(rogue_dhcp_server_IP)
          
          # The endpoint is returned for each DHCP client
          for dhcp_client in affected_dhcp_clients:
            dhcp_client_endpoint = bcf_controller.return_endpoint(endpointMAC = dhcp_client['mac'],
                                                                  endpointSegment = rogue_dhcp_server_mac_and_segment['segment'])
            ...
            if dhcp_client_endpoint.attachment_point_state != 'static' and dhcp_client_endpoint.is_ip_here(dhcp_client['ip']):
              if dhcp_client_endpoint.make_me_static(session_cookie = bcf_controller.session_cookie,
    ...
    bcf_controller.api_logout(bcf_controller.session_cookie)
